...
function file_batch()
{
	...
	fobj.method = "post";
	fobj.action = "../design/codi/indb.jsp?mode=batch";
	fobj.submit();
}
...
{
	var path = sobj.getAttribute("value");
	alert("path --> " + path);
	document.location.href='../design/iframe.codi.jsp?design_file=' + path;
	/*
...
DCDM = {
	...
	request: function (obj, val)
	{
		...
		var urlStr = "./_ajax.jsp?mode=getDir&idx=" + idx + "&val=" + val + "&dirfiles=" + obj.value + "&dummy=" + new Date().getTime();
...
DCCM = {
	...
	file_check: function ()
	{
		...
		var urlStr = "./_ajax.jsp?mode=chkFile&dir_name=" + dir_name + "&file_name=" + fobj.file_name.value + "&file_ext=" + fobj.file_ext.value + "&dummy=" + new Date().getTime();
		var ajax = new Ajax.Request( urlStr,
		{
			method: "get",
			onComplete: function ()
...
DCSM = {
	...
	chk: function (fobj)
	{
		...
		document.fm.action = '../design/codi/indb.jsp?mode=saveas&design_file=' + fobj.dir_name.value + fobj.file_name.value + fobj.file_ext.value;
		document.fm.submit();
	...
	file_check: function ()
	{
		...
		var urlStr = "../design/codi/_ajax.jsp?mode=chkFile&dir_name=" + fobj.dir_name.value + "&file_name=" + fobj.file_name.value + "&file_ext=" + fobj.file_ext.value + "&dummy=" + new Date().getTime();
		var ajax = new Ajax.Request( urlStr,
		{
			method: "get",
			onComplete: function ()
...
function file_del(pop_no, file_name)
{
	...
	fobj.method = "post";
	fobj.action = "../design/indb.jsp?mode=del&sno=" + pop_no+"&filename="+file_name;
	fobj.submit();
}
...
DCTM = {
	...
	write: function (t_name, t_width, t_rows, t_property, tplFile)
	{
		...
		\
		<div class="head">\
		<!--<input type="button" class="icon" style="background:url(/resources/shop/admin/img/codi/btn_webftp.gif);width:87px;" title="WebFTP 이미지 관리" onclick="DCTM.webftp();">-->\
		<input type="button" class="icon" style="background:url(/resources/shop/admin/img/codi/btn_codeinput.gif);width:40px;" title="코디소스입력" onclick="DCTM.put_codi();">\
		<input type="button" class="icon" style="background:url(/resources/shop/admin/img/codi/bu_01.gif);width:43px;" title="줄바꿈 설정/해지" onclick="DCTM.textarea_wrap();">\
		<input type="button" class="icon" style="background:url(/resources/shop/admin/img/codi/bu_04.gif);width:38px;" title="한칸씩늘리기▼" onmousedown="DCTM.row_start();DCTM.row_control( \'+\' );" onmouseup="DCTM.row_stop();" onmouseout="DCTM.row_stop();">\
		<input type="button" class="icon" style="background:url(/resources/shop/admin/img/codi/bu_03.gif);width:38px;" title="한칸씩줄이기" onmousedown="DCTM.row_start();DCTM.row_control( \'-\' );" onmouseup="DCTM.row_stop();" onmouseout="DCTM.row_stop();">\
		<input type="button" class="icon" style="background:url(/resources/shop/admin/img/codi/bu_05.gif);width:54px;" title="크게늘리기" onclick="DCTM.row_direct( 50 );">\
		<input type="button" class="icon" style="background:url(/resources/shop/admin/img/codi/bu_06.gif);width:32px;" title="기본" onclick="DCTM.row_direct( ' + t_rows + ' );">\
		<br>\
...
//		var urlStr = "/shop/admin/promotion/popup_manager/fileContents.dojson?mode=getTextarea&body=" + body + "&tplFile=" + tplFile;
//		var ajax = new Ajax.Request( urlStr,
//		{
//			method: "get",
//			onComplete: function ()
...
//				alert("req"+req);
//				if ( req.status != 200 ) return;
//				document.getElementById( body ).value = req.responseText.trim();
//			}
//		} );
		ajaxCallJson("/shop/admin/promotion/popup_manager/fileContents.dojson"
				, {
					mode:"getTextarea&body"
					, body : body
					, tplFile : tplFile
				}
				,  function(result){
					document.getElementById( body ).value = result.popUpVO.fileContents.trim();
				});
	},
	...
	webftp: function ()
	{
		var hrefStr = '/shop/admin/webFtp/popupWebFtp';
	...
	template: function ()
	{
		var hrefStr = 'http://gongji.godo.co.kr/userinterface/help_template.jsp';
...
DCRM = {
	...
	{
//		var urlStr = "../design/codi/_ajax.jsp?mode=getReplacecode&design_file=" + design_file;
		var urlStr = "/shop/admin/promotion/popup_manager/fileContents.dojson?mode=getReplacecode&design_file=" + design_file;
		var ajax = new Ajax.Request( urlStr,
		{
			method: "get",
			onComplete: function ()
