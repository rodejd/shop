...
function frame_load(){
	...
	var page_query = '?webftpid=' + webftpid +"&dPath=" + getCookie( name='dPath' );
	

	if ( dPath = getCookie( name='dPath' ) ) page_query += '&dPath=' + dPath;

	/*if ( window.top.global_frame.location.href == 'about:blank' || window.top.global_frame.location.pathname == curr_path + 'webftp_tree.jsp' ){
		window.top.global_frame.location.href = curr_path + 'webftp_tree.jsp' + page_query;
	}*/
	
	
	if ( document.getElementById('global_frame').contentWindow.location.href == 'about:blank' || document.getElementById('global_frame').contentWindow.location.pathname == curr_path + 'webftp_tree.jsp' ){
		document.getElementById('global_frame').contentWindow.location.href = '/shop/admin/webFtp/popupWebFtp/' + 'tree' + page_query;
	}
	document.getElementById('folder_frame').contentWindow.location.href = '/shop/admin/webFtp/popupWebFtp/' + 'list' + page_query;
	//window.top.folder_frame.contentWindow.location.href = curr_path + 'webftp_list.jsp' + page_query;
}
...
function dPathCookie( dPath ){
	...
	//parent.document.getElementById('folder_frame').contentWindow.location.reload();
	
	
	//test해보자
	/*var page_query = '?webftpid=' + webftpid + '&dPath=' + dPath;
	//window.top.folder_frame.location.href = curr_path + 'webftp_list.jsp' + page_query;
	parent.document.getElementById('folder_frame').contentWindow.location.href = curr_path + 'webftp_list.jsp' + page_query;
	window.top.folder_frame.location.reload();*/
}
...
function frame_search(){
	...
	window.top.global_frame.location.href = curr_path + 'webftp_search.jsp' + page_query;
}
...
function frame_info( file_root ){
	...
	if ( file_root == '' ){
		...
		window.top.global_frame.location.href = curr_path + 'webftp_info.jsp?webftpid=' + webftpid + '&file_root=' + file_root;
	}
}
...
function frame_list_dpath( dPath ){
	...
	if ( window.top.folder_frame ){
		...
		parent.document.getElementById('folder_frame').contentWindow.location.href = "/shop/admin/webFtp/popupWebFtp/" + 'list' + page_query;
...
function urlCopyact( urlObj ){
	...
	if ( !document.all ){
		...
		alert('인터넷 익스플로러에서만 지원되는 기능입니다.');
		return false;
	}

	var doc = urlObj.createTextRange();
...
function mkdir(){
 	...
 	var win = Pubwinopen_return( curr_path + 'webftp_mkdir.jsp?webftpid=' + webftpid, 'win_mkdir', 500, 500, 10, 10, 0 );
	win.focus();
}
...
function file_upload(){
 	...
 	var win = Pubwinopen_return( curr_path + 'webftp_upload.jsp?webftpid=' + webftpid, 'win_upload', 700, 500, 10, 10, 1 );
	win.focus();
}
...
function file_gdcopy(){
 	...
 	var win = Pubwinopen_return( curr_path + 'webftp_gdcopy.jsp?webftpid=' + webftpid, 'win_gdcopy', 700, 500, 10, 10, 1 );
	win.focus();
}
