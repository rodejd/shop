...
function DynamicTree(id) {
	this.path = "/resources/shop/admin/img/";
	this.img = {
	...
	this.readData = function(id, el) {
		...
		if (this.local == 'category'){ // (분류관리 경우)
			/*query += "../goods/indb.jsp?mode=getCategory";*/
			query += "../goods/category.dojson";
			if (id) query += "&category=" + id;
		}
		else if (this.local == 'codi'){ // (디자인코디관리 경우)
			query += "../design/codi/_ajax.jsp?mode=getCodiTree";
			if (id) query += "&dirfiles=" + id;
		}
		
		var urlStr = query + "&dummy=" + new Date().getTime();
				
		ajaxCallJson("../goods/category.dojson"
				,{category:id}
				,function(result){
									//alert(req.responseText);
//									if ((response = req.responseText) == '') return;
									self.jsonData = eval('(' + (JSON.stringify(result.cateVO.categoryList)) + ')');
									if (id == null) {
										self.parseData(self.jsonData, self.tree, 1);
										self.loadState();
										self.updateHtml();
										if (self.category) {
											loadHistory(self.category);
										} else {
											openTree(_ID('node_top'));
										}
									}
									else if (id) {
	...
	function Node(id, text, parentNode, childNodes, isDoc, isFolder) {
		...
		this.toHtml = function() {
			...
			if (this.isFolder) {
				...
				s += '</a>';
				if (self.local == 'codi') { // (디자인코디관리 경우)

					if (this.id.match(this.text) == null && this.id != 'ahead/'){
					...
					if (this.id == 'popup/') { // 메인팝업창 목록 호출코드
						...
						s += '?<input type=hidden value="?"></a>'.format(text, '../design/iframe.popup_list.jsp');
					}
					else {
			...
			if (this.isDoc) {
				...
				if (self.local == 'codi') { // (디자인코디관리 경우)
					...
					if (this.id.match(/^popup\//)){
						s += '?<input type=hidden value="?"></a>'.format(text, '../design/iframe.popup_register.jsp?file=' + this.id.replace(/^popup\//, ''));
					}
					else if (this.id == 'style.css') s += '?<input type=hidden value="?"></a>'.format(text, '../design/iframe.css.jsp');
					else if (this.id == 'common.js') s += '?<input type=hidden value="?"></a>'.format(text, '../design/iframe.js.jsp');
					else if (this.id == 'main/intro.htm') s += '?<input type=hidden value="?"></a>'.format(text, '../design/iframe.intro.jsp');
					else {
						s += '?<input type=hidden value="?"></a>'.format(text, this.id);
					}
				}
