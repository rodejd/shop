DynamicTree.prototype.Shifting = function () {
	function shifting() {
		...
		this.mcdMvUp = function (evt) {
			...
			var urlStr = "../goods/indb.jsp?mode=chgCategoryShift" + query + "&dummy=" + new Date().getTime();
...
/*			var ajax = new Ajax.Request( urlStr,
		...
		this.createSection = function (id) {
			...
			vimg.height = "18";
			vimg.alt = "";
			node.getElementsByTagName('IMG')[1].src = self.img.folderOpen; // forder icon 변경
			node.getElementsByTagName('IMG')[1].setAttribute('id', id + '-icon');
		...
		this.dropSection = function (id) {
			...
			vimg.height = "18";
			vimg.alt = "";
			node.getElementsByTagName('IMG')[1].src = self.img.doc;
		};
	}
