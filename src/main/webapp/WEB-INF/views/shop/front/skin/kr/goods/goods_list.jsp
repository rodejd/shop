...
$(document).ready(function(){
	...
	var totalWidth = 0;
	var liSize = $('.liSize');
	liSize.each(function(){
		totalWidth = totalWidth + Math.ceil($(this).width());
	})
	$('.ulSize').css('width', totalWidth+2)

});
</script>

<form name="frmList" id="frmList" method="get" action="${ctx}/shop/goods/goods_list" style="margin-bottom:0;">
	...
	<div class="global-wrapper clearfix" id="global-wrapper">
		<div class="top_tit_ty01">
			<div class="tit_dp01">${currPosition}</div>
		</div>
		...
		<div class="x-goods-list container_list">
			<c:if test="${frontGoodsVO.frontGoodsGoodsCategoryList ne '' and fn:length(frontGoodsVO.frontGoodsGoodsCategoryList) > 0}">
				<div class="tab_ty01">
					<ul class="ulSize"><!-- 이곳에 모바일일때 li의 값들이 더해져서 총 길이가 style="width:0000px;" 로 들어가면 됩니다 -->
						<c:forEach var="ctList" items="${frontGoodsVO.frontGoodsGoodsCategoryList }" varStatus="status">
							<li class="liSize"><a href="${ctx}/shop/goods/goods_list?category=${ctList.category}" class="${frontGoodsVO.category eq ctList.category ? 'on' : ''}">${ctList.catnmKR}</a></li>
						</c:forEach>
					</ul>
				</div>
			</c:if>
								...
								<div class="in_cate_list pr_bx_in">
									...
									<ul>
										<c:forEach var="ctList" items="${frontGoodsVO.frontGoodsGoodsCategoryList }" varStatus="status">
											<li class="dp2">
												<div class="s_tit_n03">
													<a href="${ctx}/shop/goods/goods_list?category=${ctList.category}">${ctList.catnmKR} (${stringUtil:getMoneyFormatInteger(ctList.goodsCnt)})</a>
												...
												<c:if test="${ctList.categoryList ne ''}">
													<ul class="bx_dp3">
														<c:forEach var="subCtList" items="${ctList.categoryList}" varStatus="substatus">
															<li class="dp3"><a href="${ctx}/shop/goods/goods_list?category=${subCtList.category}">${subCtList.catnmKR} (${stringUtil:getMoneyFormatInteger(subCtList.goodsCnt)})</a></li>
														</c:forEach>
													</ul>
												</c:if>
											</li>
			...
			<div class="bx_total_num">
				총 ${stringUtil:getMoneyFormatInteger(frontGoodsVO.rowCount)}개의 상품 검색
			</div>

			<div class="sub_list02">
				<ul>
					<c:forEach var="gdList" items="${frontGoodsVO.frontGoodsGoodsList }" varStatus="status">
						<li>
							<div class="in_bx">
								<div class="bx_thum">
									<div class="">
