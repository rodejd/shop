...
<form name="frmList" id="frmList" method="get" action="${ctx}/shop/goods/hot">
	<input type="hidden" name="schType" value="${frontGoodsVO.schType}">
	...
	<input type="hidden" name="pageSize" value="${frontGoodsVO.pageSize}">
 	...
 	<div class="global-wrapper clearfix" id="global-wrapper">
		<div class="top_tit_ty01">
			<div class="tit_dp01">HOT 100</div>
			...
			<!-- t100 rolling -->
			<div>
				<div class="swiper-container top100_rol">
					<div class="swiper-wrapper">
						<c:forEach var="sdList" items="${frontGoodsVO.slideGoodsList}" varStatus="status">
							<div class="swiper-slide">
								<div class="top100_tp_list">
									<div class="thum_bx"><img src="${fn:replace(sdList.imgs,'|','')}" alt="${sdList.goodsnmKR}" style="width:100%;" /></div>
									<div class="bx_info">
										<div class="tx_brand">${sdList.brandnm}</div>
										<div class="tx_tit">${sdList.goodsnmKR}</div>
										<div class="tx_price01">${shopLibFunction:getExchange(sdList.price, wskin)}<span class="tx_per"><c:if test="${sdList.priceRate >= 3}"><fmt:formatNumber pattern="#0" value="${sdList.priceRate}"/>%</c:if></span></div>
										<div class="tx_price02"><c:if test="${sdList.priceRate >= 3}">${shopLibFunction:getExchange(sdList.consumer, wskin)}</c:if></div>
									</div>
								</div>
							</div>
						</c:forEach>
				...
				<script>
					var swiper = new Swiper('.top100_rol', {
						slidesPerView: 'auto',
						spaceBetween: 0,
						pagination: {
		...
		<div class="x-goods-list  container_list">
			<div class="bx_filter02">
				<div class="bx_l"> ${stringUtil:getMoneyFormatString(frontGoodsVO.rowCount)}개의 상품 검색</div>
				<div class="bx_r">
					<div class="in_bx_ft">필터</div>
				</div>

				<!-- 필터 -->
				<div class="ft_n_wrap ty02">
			...
			<div class="hot100_tab">
				<ul>
					<li>
						<a href="javascript:schType('');">
							<div class="bx_cir<c:if test="${empty frontGoodsVO.schType or frontGoodsVO.schType eq ''}"> on</c:if>">ALL</div>
							<p class="bt_tx">전체</p>
						</a>
					</li>
					<li>
						<a href="javascript:schType('clothing');">
							<div class="bx_cir<c:if test="${frontGoodsVO.schType eq 'clothing'}"> on</c:if>"><img src="/resources/shop/data/skin/kr/images/img_tab01_02.png" alt="" /></div>
							<p class="bt_tx">의류</p>
						</a>
					</li>
					<li>
						<a href="javascript:schType('shoes');">
							<div class="bx_cir<c:if test="${frontGoodsVO.schType eq 'shoes'}"> on</c:if>"><img src="/resources/shop/data/skin/kr/images/img_tab01_03.png" alt="" /></div>
							<p class="bt_tx">신발</p>
						</a>
					</li>
					<li>
						<a href="javascript:schType('bags');">
							<div class="bx_cir<c:if test="${frontGoodsVO.schType eq 'bags'}"> on</c:if>"><img src="/resources/shop/data/skin/kr/images/img_tab01_04.png" alt="" /></div>
							<p class="bt_tx">가방</p>
						</a>
					</li>
					<li>
						<a href="javascript:schType('accessories');">
							<div class="bx_cir<c:if test="${frontGoodsVO.schType eq 'accessories'}"> on</c:if>"><img src="/resources/shop/data/skin/kr/images/img_tab01_05.png" alt="" /></div>
							<p class="bt_tx">악세사리</p>
						</a>
					</li>
				</ul>
			</div>

			<div class="sub_list03">
 				<ul>
					<c:forEach var="gdList" items="${frontGoodsVO.frontGoodsList}" varStatus="status">
						...
						<c:if test="${status.count % 2 eq 0 or status.last}">
							<div style="clear:both;"></div>
						</c:if>
					</c:forEach>
				</ul>
