...
<%-- <script type="text/javascript" src="/resources${shopLibFunction:webSkin(pageContext.request.requestURL)}/js/board/webzine.js"></script> --%>
...
<div class="x-list-webz container">
    	...
    	<div class="tab-content nb">
			...
			<div id="content-area" class="content-wrap">
				...
				<div class="content">
					...
					<fieldset id="" class="clear-default mt40">
						<div class="sub-sch-area">
							...
							<ul class="category-selections clearfix">
			                    ...
			                    <li>
			                    	<button type="button" class="btn btn-primary" onclick="fn_submit()"><span class="txt-white">검색</span></button>
			                    </li>
					...
					<article class="product-review">
			            ...
			            <div>
			                <h5 class="product-review-title">
		                		...
		                		<c:choose>
				                <c:when test="${(level ne '99') and (list.secret eq 'o')}"><a href="#nav-dialog" data-effect="mfp-move-from-top" class="popup-text" onclick="$('#rtlno').val('${rtListN.no}');"></c:when>
			                ...
			                <div class="product-review-body" style="margin-left:50px;margin-top:20px;">${list.contents}</div>
			                <ul class="list-inline product-review-actions" style="margin-left:50px;margin-top:20px;">
                                <li><a href="#">작성자 : ${list.name}</a>
                                </li>
                                <li><a href="#">작성일 : <fmt:formatDate value="${list.regdt}" pattern="yyyy.MM.dd HH:mm"/></a>
                                </li>
					...
					<article class="product-review">
			            <div class="product-review-author" style="margin-left:20px;">${(boardVO.rowCount - status.index) - ( boardVO.rowStart ) }</div>
			            <div>
			                <h5 class="product-review-title">
			                	<c:if test="${ boardVO.brdSetupObj.bdUseSubSpeech != ''}">[${list.category}] </c:if>
				                <c:choose>
			                ...
			                <div class="product-review-body" style="margin-left:50px;margin-top:20px;">${list.contents}</div>
			                <ul class="list-inline product-review-actions" style="margin-left:50px;margin-top:20px;">
                                <li><a href="#"><i class="fa fa-user"></i> ${list.name}</a>
                                </li>
                                <li><a href="#"><i class="fa fa-clock-o"></i><fmt:formatDate value="${list.regdt}" pattern="yyyy.MM.dd HH:mm"/></a>
                                </li>
                                <li><a href="#"><i class="fa fa-hand-o-up"></i> ${list.hit}</a>
                                </li>
                                <c:if test="${boardVO.brdSetupObj.bdUseFile eq 'on'}">
                                <c:if test="${list.newfile != ''}">
                        		<c:forTokens var="st" items="${list.newfile}" delims="|">
                                <li><a href="#"><i class="fa fa-file-text"></i> </a><a href="javascript:void(0);" onclick="fileDownload('${st}');">${st}</a>
                                </li>
                                </c:forTokens>
                                </c:if>
                                </c:if>
