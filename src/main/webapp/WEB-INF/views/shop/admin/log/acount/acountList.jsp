 ...
 if((msg != null) && (msg != '')){
    ...
    function sendAcount(){
		...
		if($("input:checkbox[name=all]").is(":checked") == true){
			console.log("allcheck on ");
			$('[name=all]').prop('checked', false);
		}
    	
    		acountChks = $("#acountList input:checkbox:checked");
    		chkSend = $("#chkSend")
    		
    		
    		if(0 >= acountChks.length){
    			alert("적용하기 원하시는 데이터에 체크해주세요.");
    			return false;
    		}
    		
    		flag = $("#acountFlag option:selected").val();
    		if(flag == ""){
    			alert("목록에서 선택값을 주셔야합니다.");
    			return false;
    		}
    		
    		var acountConfirm = confirm('선택한 목록을 적용하시겠습니까?');
    		
    		if(acountConfirm){
    			/*체크된 박스 값에 데이터 넣기*/
    			acountChks.each(function(){
    				chkSend.append("<input type='hidden' name='acountArr_goodsno' value='"+$(this).data("goodsno")+"'/>");
    				chkSend.append("<input type='hidden' name='acountArr_ordno' value='"+$(this).data("ordno")+"'/>");
    			});
    		
    			chkSend.append("<input type='hidden' name='acountFlag' value='"+flag+"'/>");
						...
						<table class="tb" border="1" bordercolor="#e6e6e6" cellpadding="5" style="width: 100%; border-collapse: collapse;">
							...
							<tbody>
								<tr>
									<td>판매사명</td>
									<td>
										<input type="text" name="sellernm" id="schSellerNm" value="${acountVO.sellernm }" class="line" style="height:22px">
										<input type="button" value="판매사 조회" onclick="popupLayer('${ctx}/shop/admin/common/sellerSearchPopup', 600, 500);">
										<input type="hidden" name="sellerCd" id="schSellerCd" value="${acountVO.sellerCd }">
									</td>
								</tr>
								<tr>
									<td>검색어</td>
						...
						<table id="" width="100%" cellpadding="0" cellspacing="0" border="0">
							...
							<tbody>
								...
								<c:forEach var="i" begin="0" end="5" step="1">
									<td class="center">₩<fmt:formatNumber value="${acountVO.total[i]}" pattern="#,###"/> </td>
								</c:forEach>
								</tr>
...
<!-- 표 -->
						<table class=tb id="acountList" width="100%" cellpadding="4" cellspacing="0" border="0">
								<tr class="rndbg">
									<td class="noline">
										<input type="checkbox" name="all" class="allcheck"/>
									</td>								
									<th>입점업체</th>
									<th>주문번호</th>
									<th>주문일자</th>
								...
								<c:forEach items="${acountVO.acountList}" var="acountList" varStatus="status">
									<tr height="25">
										<td class="noline">
										<c:choose>
											<c:when test="${acountList.accountFlag == '정산완료'}">
												<input type="checkbox" name="chk[]" disabled />		 
											</c:when>
											<c:otherwise>
												<input type="checkbox" name="chk[]" class="allchk" data-goodsno="${acountList.goodsno}"
													data-ordno="${acountList.ordno}" />
											</c:otherwise>										
										</c:choose>	
										</td>							
										<td class="center"><font class=ver81 color=444444>${acountList.sellernm}</td>
								...
								<tr><td colspan="15" class="rndline"></td></tr>
								<tr><td colspan="15">
									<select id="acountFlag">
										<option value="">전체</option>
										<option value="W">정산대기</option>										
										<option value="I">정산중</option>
										<option value="E">정산완료</option>
									</select>
									<input type="button" value="일괄적용" onclick="sendAcount()">
								</td></tr>
						</table>
