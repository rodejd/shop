...
<script>
	...
	function chkCancel(m){
		if($("#layer_cancel").css("display") == "none"){
			...
			*/
			var url = "/shop/admin/order/orderCancel?m="+m+"&ordno=${popupVO.ordno}&mno=${popupVO.rtData.mno}&sno=" + sno.join();
...
<form name=frmOrder action="${ctx}/shop/admin/order/indb.cancel" method=post>
	<input type=hidden name=mode value="${popupVO.rtData.step2 >= 79 ? 'cancelOrder' : 'modOrder'}">
	...
	<div class=button  >
		...
		<c:choose>
			<c:when test="${ 'view' == param.viewFlg }">
				<a href="${ctx}/shop/admin/order/list?mode=group&period=0&first=1"><img src="/resources/shop/admin/img/btn_list.gif"></a>
			</c:when>
			<c:otherwise>
