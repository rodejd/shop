	...
	<table class="tb" cellpadding="5" cellspacing="0" border="1" bordercolor="#e6e6e6" style="width: 100%; border-collapse: collapse;">
		<tbody>
			<tr height="25" bgcolor="#2E2B29" class="small4" style="padding-top:8px">
				...
				<!-- <th><font color="white">회원할인</font></th> -->
				<!--<th> <font color="white">쿠폰할인</font> </th>-->
				<th><font color="white">결제가격</font></th>
			...
			<c:when test="${printVO.rtList != null && printVO.rtListSize > 0 }">
				<c:forEach items="${printVO.rtList}" var="rt" varStatus="status">
					<tr bgcolor="#ffffff">
						...
						<td nowrap>${rt.ea}</td>							
						<td nowrap>${rt.price}</td>
						
						<%-- <td width=55 nowrap><fmt:formatNumber value="${rt.memberdc}" pattern="#,###"/></td> --%> <!-- 회원할인 보여주지 않게 주석처리 -->
						<%-- <td width=55 nowrap><fmt:formatNumber value="${popupVO.rtData.settleprice}" pattern="#,###"/> </td>--%> <!-- 쿠폰할인 부분 주석처리 -->
						<td width=55 nowrap><fmt:formatNumber value="${printVO.rtData.prnsettleprice}" pattern="#,###"/></td>
						
						<td nowrap>
							<%-- 기존 소계 ${rt.supply} 이거를 상품+추가옵션 으로 소계를 수정함 --%>
							${(rt.price* rt.ea)+rt.addoptPrice}
						</td>
	...
	<c:if test="${'0' != printVO.deliveryBasis }">
		<div style="padding:5px 0 0 12px">
			<c:if test="${printVO.rtData.step > 0}">
				<a href="#"><img src="/resources/shop/admin/img/btn_input_delinumber.gif" border=0></a>
			</c:if>
		</div>
	</c:if>
	<!-- 개별송장입력 끝 -->
	...
	<!--<col class=cellC><col class=cellL width=120><col class=cellL>-->
		<tr>
			...
			<td width=110 align=right>
				₩<font class=ver90><fmt:formatNumber value="${printVO.goodsPrice + printVO.rtData.delivery + printVO.rtData.addDelivery + printVO.rtData.eggfee}" pattern="#,###"/></font>
			</td>
			<td><img src="/resources/shop/admin/img/arrow_gray.gif" align=absmiddle><font class=small color=444444>상품가격</font> (₩<font color=0074BA class=ver81><fmt:formatNumber value="${printVO.goodsPrice}" pattern="#,###"/></font>)
				<c:if test="${printVO.rtData.delivery != 0}">
					+ 배송비 (<font color=0074BA class=ver81>₩<fmt:formatNumber value="${printVO.rtData.delivery}" pattern="#,###"/></font>)
				</c:if>
				<c:if test="${printVO.rtData.addDelivery != 0}">
					+ 추가배송비 (<font color=0074BA class=ver81>₩<fmt:formatNumber value="${printVO.rtData.addDelivery}" pattern="#,###"/></font>)
				</c:if>
				<%-- <c:if test="${printVO.rtData.eggfee != 0}">
					+ 보증보험수수료 (<font color=0074BA class=ver81>₩<fmt:formatNumber value="${printVO.rtData.eggFee}" pattern="#,###"/></font>)
				</c:if> --%>
			</td>
		</tr>
		<tr>
			<td align=center bgcolor=#F6F6F6>할인액</td>
			<td align=right><font class=ver90>- ₩<fmt:formatNumber value="${printVO.discount}" pattern="#,###"/></font></td>
			<td>
				<img src="/resources/shop/admin/img/arrow_gray.gif" align=absmiddle>
				<font class=small color=444444>
					<c:if test="${printVO.memberDC > 0 }">	
						회원할인 (<font color=0074BA class=ver81>₩<fmt:formatNumber value="${printVO.memberDC}" pattern="#,###"/></font>)
					</c:if>
					
					<c:if test="${printVO.memberDC > 0 and (printVO.rtData.coupon > 0 or printVO.rtData.emoney > 0)}">	
						+
					</c:if>
					
					<c:if test="${printVO.rtData.coupon > 0}">
						 쿠폰할인 (<font color=0074BA class=ver81>₩<fmt:formatNumber value="${printVO.rtData.coupon}" pattern="#,###"/></font>)
					</c:if>
					
					<c:if test="${(printVO.memberDC > 0 or printVO.rtData.coupon > 0) and printVO.rtData.emoney > 0}">
						+
					</c:if>
					
					<c:if test="${printVO.rtData.emoney > 0}">
						적립금사용 (<font color=0074BA class=ver81>₩<fmt:formatNumber value="${printVO.rtData.emoney }" pattern="#,###"/></font>)
					</c:if>
					 <%-- 에누리 ₩<input type=text name=enuri value="${printVO.rtData.enuri }" size=6 class='ver81 right' style='color:#0074BA'> --%>
				</font>
			</td>
		</tr>
		<tr>
			<td align=center bgcolor=#F6F6F6>결제금액</td>
			<td align=right>₩<font color=0074BA class=ver8><b><fmt:formatNumber value="${printVO.rtData.prnsettleprice}" pattern="#,###"/></b></font></td>
			<td>
				<font class=small color=444444>
					<!-- 사용자가 했갈리는걸 방지하기위해 의미없는 부분이라 생각되어 주석처리함 -->
					<%-- <c:if test="${printVO.settleprice != printVO.rtData.settleprice }">
						<img src="../img/arrow_gray.gif" align=absmiddle>최초주문금액  (₩<font color=0074BA class=ver81><fmt:formatNumber value="${printVO.rtData.settleprice}" pattern="#,###"/></font>)
				 		+ 취소금액 합계 (₩<font color=0074BA class=ver81><fmt:formatNumber value="${printVO.settleprice - printVO.rtData.settleprice}" pattern="#,###"/></font>)
					</c:if> --%>
				</font>
			</td>
		</tr>
		<tr>
			...
			<td align=right>
				<%-- <select name=step>
					${printVO.stepSelectOption}
				</select> --%>
				${shopLibFunction:r_stepi(printVO.rtData.step,printVO.rtData.step2)}
			</td>
			<td>
			</td>
		</tr>
				...
				<td>
					...
					<c:forEach items="${printVO.tmpRt}" var="tmpRt" varStatus="status">
						<tr>
							...
							<td align=center><font class=ver8>₩${tmpRt.rfee}</font></td>
					
							<td align=center>₩<font class=ver8 color=EA0095><b>${tmpRt.rprice}</b></font></td>
					
							<td align=center>₩<font class=ver8>${tmpRt.remoney}</font></td>
		...
		<%-- 	<font color="494949">
					...
					<col class=cellC><col class=cellL>
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${printVO.rtData.step >= 1 && printVO.rtData.step < 4}">
