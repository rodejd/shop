...
<script language="javascript">
	...
	function dnXls(mode){
		...
		$('form[name=frmDnXls]').attr('action', ctx+"/shop/admin/myritz/orderExcelDown");
	...
	<table class="tb">
		...
		<tr>
			...
			<td colspan="3" class="noline">
				...
				<div style="float: left; padding-right: 10px">
					<input type="checkbox" name="step2" value="1" ${ fn:contains(orderFM.step2,'1') ? 'checked' : ''} /><font class="small" color="5C5C5C">주문취소</font>
				</div>
		...
		<tr>
			<td>판매사명</td>
			<td>
				<input type="text" name="schMyritzNm" id="schMyritzNm" value="${orderFM.schMyritzNm}" class="line" style="height:22px" />
				<input type="button" value="판매사 조회" onclick="popupLayer('${ctx}/shop/admin/common/myritzSearchPopup', 600, 500);" />
				<input type="hidden" name="schMyritzCd" id="schMyritzCd" value="${orderFM.schMyritzCd}" />
			</td>
		</tr>
		...
		<tr>
			...
			<td colspan="3" class="noline">
				<font class="small1" color="5C5C5C">
					...
					<input type="checkbox" name=couponyn value="1"/>쿠폰사용
					<input type="checkbox" name=cashreceipt value="1"/>현금영수증
				</font> 
...
<form name="frmList" method="post" action="${ctx}/shop/admin/order/indb" onsubmit="return chkForm(this)">
...
<c:forEach items="${orderFM.orderList}" var="list" varStatus="i">
	...
	<c:set var="stepMsg"  value=" ${shopLibFunction:getStepMsg(list.step, list.step2, list.ordno, '')} " />
	<c:set var="stMsg"  value=" ${shopLibFunction:getStepMsg(list.step, list.step2, list.ordno, '')} " />
	
	<c:if test="${fn:length(stMsg) > 6 }">
		<c:set var="stMsg"  value=" ${fn:substring(stMsg, 0, 5) } " />
	</c:if>
	...
	<c:if test="${ st1 != list.step or st2 != list.step2 }">
	...
	<tr>
		...
		<td height="30" colspan="9" align="right" style=padding-right:8>합계: <font class="ver9"><b>₩${stringUtil:getMoneyFormatInteger( totmoney )}</b></font></td>
		<td colspan="3"></td>
	</tr>
	...
	<!-- 주문흐름보기 끝 -->
		...
		<tr height="25" bgcolor="${bgcolor}" bg="${bgcolor}" align="center">
			...
			<td align="left">
				<a href="${ctx}/shop/admin/myritz/myritzPopupOrder?ordno=${list.ordno}&viewFlg=view"><font class="ver81" color="0074BA"><b>${list.ordno}</b></font></a>
				<a href="javascript:popup('${ctx}/shop/admin/myritz/myritzPopupOrder?ordno=${list.ordno}',800,600)"><img src="/resources/shop/admin/img/btn_newwindow.gif" border="0" align="absmiddle"></a>
			</td>
			<td align="left">
				<div style="height: 13px; overflow-y: hidden;">
					<c:if test="${ list.oldordno != '' }">
						<a href="javascript:popup('${ctx}/shop/admin/myritz/myritzPopupOrder?ordno=${list.ordno}',800,600)"><img src="/resources/shop/admin/img/icon_twice_order.gif"></a>
					</c:if>
					<c:if test="${ list.escrowyn == 'y' }">
						<a href="javascript:popup('${ctx}/shop/admin/myritz/myritzPopupOrder?ordno=${list.ordno}',800,600)"><img src="/resources/shop/admin/img/btn_escrow.gif"></a>
					</c:if>
					<c:if test="${ list.eggyn == 'y' }">
						<a href="javascript:popup('${ctx}/shop/admin/myritz/myritzPopupOrder?ordno=${list.ordno}',800,600)"><img src="/resources/shop/admin/img/icon_guar_order.gif"></a>
					</c:if>
					...
					<font class="small1" color="444444">
						...
						<c:choose>
							<c:when test="${ list.goodsnmcnt > 1 }">
								[판매사 : ${list.myritzNm }] 외  ${ list.goodsnmcnt - 1}
							</c:when>
							<c:otherwise>
								[판매사 : ${list.myritzNm }]
							</c:otherwise>
			...
			<td>
				<c:if test="${ list.mid != '' }">
					...
					<font class="small1" color="0074BA"><b>${list.nameorder}</b>
						<c:if test="${ list.mid != '' }">
							( ${list.mid} )
						</c:if>
					</font>
			...
			<td class="ver81"><b>${stringUtil:getMoneyFormatInteger( fn:trim(list.prnsettleprice) )}</b></td>
...
<c:otherwise>
		...
		<tr bgcolor="#f7f7f7" height="30">
			<td colspan="10" align="right" style="padding-right: 8">합계 : <span class="ver9"><b>₩${stringUtil:getMoneyFormatInteger( totmoney )}</b></span></td>
			<td colspan="3"></td>
		</tr>
		<tr bgcolor="#f7f7f7" height="30">
			<td colspan="10" align="right" style="padding-right:8">전체합계 : <span class="ver9"><b>₩${stringUtil:getMoneyFormatInteger( tmp_amt )}</b></span></td>
			<td colspan="3"></td>
		</tr>
...
<!-- 				<td><a href="javascript:dnXls('erp')"><img src="dss" border="0"></a></td> -->
			...
			<tr align="center">
				<td><a href="javascript:popupLayer('${ctx}/shop/admin/order/popup.orderxls?mode=orderXls',550,700,'popLayerCallback');"><img src="/resources/shop/admin/img/btn_order_data_order_ot.gif" border="0"></a></td>
				<td><a href="javascript:popupLayer('${ctx}/shop/admin/order/popup.orderxls?mode=orderGoodsXls',550,700, 'popLayerCallback');"><img src="/resources/shop/admin/img/btn_order_data_goods_ot.gif" border="0"></a></td>
			</tr>
		</table>
	</td>
</tr>
