...
function chk_status_modify(statVal, mode){
	...
	if(confirm(("open_modify" == mode) ? "게시/게시취소 하시겠습니까?" : "승인/반려 하시겠습니까?")){
		eventChks.each(function(){
			...
			statusModifyFrm.append("<input type='hidden' name='myritzcodeArr' value='"+$(this).data("myritzcode")+"'/>");
		});
...
================================================================================ --%>
		...
		<table width=100% cellpadding=0 cellspacing=0 border=0>
			<tr>
				<td>
					<form name="eventSchForm" id="eventSchForm" action="${ctx}/shop/admin/myritz/myritzEventList" method="get">
						...
						<table class=tb>
							...
							<tr>
								...
								<td colspan=3>
									<input type="text" name="schMyritzNm" id="schMyritzNm" value="${eventVO.schMyritzNm}" class="line" style="height:22px" />
									<input type="button" value="판매사 조회" onclick="popupLayer('${ctx}/shop/admin/common/myritzSearchPopup', 600, 500);" />
									<input type="hidden" name="schMyritzCd" id="schMyritzCd" value="${eventVO.schMyritzCd}" />
								</td>
							</tr>
						</table>
		...
		<form id="statusModifyFrm" method="post" action="myritzEventIndb2">
	...
	<c:forEach items="${eventVO.eventList}" var="eventList" varStatus="st">		
			...
			<tr height=25>
				<td align=center style="padding-bottom:9">
					<font class=ver81>
						<input type="checkbox" name="chk[]" value="${eventList.sno}" data-myritzcode="${eventList.myritzcode}" onclick="">
					</font>
				</td>
				...
				<td align=center style="padding-bottom:9"><font class=ver81>${eventList.myritzname}</td>
				...
				<td align=center style="padding-bottom:4">
					<a href="myritzEventRegister?mode=modify&sno=${eventList.sno}"><img src="/resources/shop/admin/img/i_edit.gif" border=0></a>
					<a href="myritzEventIndb?mode=delete&sno=${eventList.sno}&old_attach=${eventList.attach}" onclick="return confirm('정말로 삭제하시겠습니까?\n\n이벤트내용에 쓰인 이미지는 다른 곳에서도 사용하고 있을 수 있으므로 자동 삭제되지 않습니다. \n\'디자인관리 > webFTP이미지관리 > data > editor\'에서 이미지체크 후 삭제관리하세요.')"><img src="/resources/shop/admin/img/i_del.gif"></a>
				</td>
			</tr>
	</c:forEach>	
</c:if>	
