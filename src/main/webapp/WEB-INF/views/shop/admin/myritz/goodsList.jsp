	...
	function chk_status_modify(statVal, mode){
		...
		/* if("approvalstatus_modify" == mode){
			for(i = 0; i < goodsChks.length ; i++){
				if( "" == $.trim($(goodsChks[i]).data("myritzcd")) || 1 != $(goodsChks[i]).data("aprlstat") ){
		...
		if(confirm(("open_modify" == mode) ? "게시/게시취소 하시겠습니까?" : "승인/반려 하시겠습니까?")){
			goodsChks.each(function(){
				...
				statusModifyFrm.append("<input type='hidden' name='myritzcodeArr' value='"+$(this).data("myritzcd")+"'/>");
			});
...
<form name="frmList" action="${ctx}/shop/admin/myritz/goodsList">
	...
	<table class=tb>
		...
		<tr>
			<td>판매사명</td>
			<td>
				<input type="text" name="schMyritzNm" id="schMyritzNm" value="${goodsVO.schMyritzNm}" class="line" style="height:22px" />
				<input type="button" value="판매사 조회" onclick="popupLayer('${ctx}/shop/admin/common/myritzSearchPopup', 600, 500);" />
				<input type="hidden" name="schMyritzCd" id="schMyritzCd" value="${goodsVO.schMyritzCd}" />
			</td>
		</tr>
		<tr>
			<td>상품가격</td>
		...
		<col width=40 span=2 align=center>
			...
			<tr height="25" >
				<td class="noline" align="center">
					<input type="checkbox" name="chk[]" value="${goodsList.goodsno}" data-myritzcd="${goodsList.myritzCd}" data-aprlstat="${goodsList.approvalStatus}" />
				</td>
				...
				<td align=center><font class=ver81 color=444444>${goodsList.myritzNm }</td>
				...
				<td align=center>
					<c:if test="${!empty goodsList.myritzCd}">
