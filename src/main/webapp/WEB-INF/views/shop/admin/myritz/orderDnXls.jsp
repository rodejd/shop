...
<table border="1">
	...
	<c:forEach items="${orderExcelVO.orderList}" var="list" varStatus="i">
			...
			<c:forEach items="${selConfList}" var="arry">
				<c:if test="${arry[2] == 'y'}">
					<c:choose>
						...
						<c:when test="${arry[1] == 'settleprice'}"><td>${list.settleprice}</td></c:when>
						<c:when test="${arry[1] == 'orddt'}"><td>${dateUtil:formatDate(list.orddt, "yyyy-MM-dd")}</td></c:when>
						<c:when test="${arry[1] == 'step'}"><td>${shopLibFunction:getStepMsg(list.step, list.step2, list.ordno,"")}</td></c:when>
						...
						<c:when test="${arry[1] == 'myritzNm'}"><td>${list.myritzNm}</td></c:when>
			...
			<c:forEach items="${selConfList}" var="arry">
				<c:if test="${arry[2] == 'y'}">
					<c:choose>
						...
						<c:when test="${arry[1] == 'sprice'}"><td>${list.settleprice}</td></c:when>
						<c:when test="${arry[1] == 'deliveryno'}"><td>${list.deliveryno}</td></c:when>
						<c:when test="${arry[1] == 'deliverycode'}"><td>${list.deliverycode}</td></c:when>
						<c:when test="${arry[1] == 'ddt'}"><td>${dateUtil:formatDate(list.ddt, "yyyy-MM-dd HH:mm:ss")}</td></c:when>
						<c:when test="${arry[1] == 'myritzNm'}"><td>${list.myritzNm}</td></c:when>
