...
<script language="javascript">
	...
	function goBack() {
		...
		if("list" == $("#view").val()) {
			addr = "../myritz/list?menuKey=122";
		} else {
			addr = "../myritz/confirmList?menuKey=125";
		}
	...
	$(document).ready(function() {
		...
		$("#btn_chk_id").on("click", function() {
			...
			ajaxCallJsonPost("/shop/admin/myritz/chkId.dojson", "fmList", function(result) {
				if(result) {
		...
		// 판매사코드 중복체크 클릭시
		$("#btn_chk_myritzCd").on("click", function() {
			
			if ("" == $("[name='myritzCd']").eq(0).val() || null == $("[name='myritzCd']").eq(0).val() ) {
				alert("판매사 코드를  먼저 입력하세요.");
				$("[name='myritzCd']").eq(0).focus();
				return false;
			}
			
			ajaxCallJsonPost("/shop/admin/myritz/chkMyritzCd.dojson", "fmList", function(result) {
				if(result) {
					var sResult = JSON.stringify(result);
					var pResult = JSON.parse(sResult);
		...
		/* $("#btn_reset_pwd").on("click", function() {
			...
			ajaxCallJsonPost("/shop/admin/myritz/resetPwd.dojson", "fmList", function(result) {
				if(result) {
		...
		$("#btn_save_id").on("click", function() {
			...
			if($("[name='myritzNm']").eq(0).val() == "") {
				alert("판매사명은 필수 입력사항 입니다.");
				$("[name='myritzNm']").eq(0).focus();
				return;
			} else if($("[name='myritzCd']").eq(0).val() == "") {
				...
				$("[name='id']").eq(0).focus();
				return;
			} else if($("[name='myritzPw']").eq(0).val() == "") {
				alert("판매사 비밀번호는 필수 입력사항 입니다.");
				$("[name='myritzPw']").eq(0).focus();
				return;
			} else if($("[name='chk_myritzCd']").eq(0).val() == "" || $("[name='chk_myritzCd']").eq(0).val() == "0") {
				...
				var myritzNm = $("[name='myritzNm']").eq(0).val();
				var myritzCd = $("[name='myritzCd']").eq(0).val();
				var id       = $("[name='id']").eq(0).val();
				var status   = $("[name='status']").eq(0).val();
				
				$("#myritzCd").val(myritzCd);
				$("#myritzNm").val(myritzNm);
				$("#id").val(id);
				$("#status").val(status);
				$("#fmList").attr("action", "../myritz/saveId");
				$("#fmList").attr("onsubmit", "");
				$("#fmList").submit();
	...
	<div>
		<table class=tb border="1" bordercolor="#e6e6e6" cellpadding="5" style="width: 100%; border-collapse: collapse;">
			...
			<tr>
				...
				<td colspan=2><input type="text" style="width : 100%;" label="판매사명" name="myritzNm" required maxlength="25"/></td>
				<th></th>
				<td></td>
			</tr>
			<tr>
				<th>판매사코드<span class="aster">*</span></th>
				<td style="border-right : 0px;"><input type="text" style="width : 70%;" label="판매사코드" name="myritzCd" required maxlength="18"/></td>
				<td style="border-left : 0px; text-align:right;">
					<input type=hidden name=chk_myritzCd id=chk_myritzCd label="아이디중복체크" required/>
					<input type="button" class="btn btn-primary" id="btn_chk_myritzCd" value="중복확인" />
				</td>
				<th>상태</th>
				<td>
			...
			<tr>
				...
				<td>
					...
					<input type="text" name="myritzPw" id="myritzPw" value="" maxlength ="20" required/>
				</td>
			</tr>
			</tbody>
		</table>
		            	...
		            	<p>
		      			...
		      			<p>
				      		<div class="col-md-6" style="padding-left:0px;">
				      			<input type="text" id="address" name="myritzAddr" readonly="readonly" class="form-control" placeholder="상세주소" title="상세주소" label="주소" required />
				      		</div>
				      		<div class="col-md-6" style="padding-left:0px;">
				      			<input type="text" id="address_sub" name="myritzAddrSub" class="form-control" placeholder="나머지주소" title="나머지 주소" label="세부주소" maxlength="100" />
				      		</div>
		      			</p>
		        	</div>
				</td>
			...
			<table class=tb border="1" bordercolor="#e6e6e6" cellpadding="5" style="width: 100%; border-collapse: collapse;">
				...
				<tr>
					...
					<td colspan=2 style="width : 30%">
						${managementFM.managementVO.myritzNm}
						<input type="hidden" style="width : 100%;" name="myritzNm" required value="${managementFM.managementVO.myritzNm}" maxlength="25"/>
					</td>
					<th></th>
					<td></td>
				</tr>
				<tr>
					<th>판매사코드<span class="aster">*</span></th>
					<td colspan=2>
						${managementFM.managementVO.myritzCd}
						<input type="hidden" style="width : 30%;" name="myritzCd" required value="${managementFM.managementVO.myritzCd}" maxlength="18"/>
					</td>
					<th>상태</th>
					<td>
						<span style="marigin-right:10px"></span>
...
<!-- 						<a class="btn btn-primary" href="javascript:chkId()" role="button">중복확인</a> -->
					...
					<th>비밀번호<span class="aster">*</span></th>
					<td>
						<input type="text" name="myritzPw" id="myritzPw" value="" maxlength ="20"/>
						<!-- <input type="button" class="btn btn-primary" id="btn_reset_pwd" value="비밀번호 초기화" /> -->
					...
					<td colspan=3>
			        	<div class="row">
			            	<p>
			      			...
			      			<p>
					      		<div class="col-md-6" style="padding-left:0px;">
					      			<input type="text" id="address" name="myritzAddr" readonly="readonly" class="form-control" placeholder="상세주소" 
					      			title="상세주소" label="주소" value="${managementFM.managementVO.myritzAddr}" required />
					      		</div>
					      		<div class="col-md-6" style="padding-left:0px;">
					      			<input type="text" id="address_sub" name="myritzAddrSub" class="form-control" placeholder="나머지주소" 
					      			title="나머지 주소" label="세부주소" value="${managementFM.managementVO.myritzAddrSub}" required maxlength="100"/>
					      		</div>
			      			</p>
			        	</div>
					</td>
...
<%-- 			<input type=image src="../img/btn_modify.gif" class="submitImg" onclick="javascript:goModify('${managementFM.managementVO.myritzCd}'); return false;" /> --%>
