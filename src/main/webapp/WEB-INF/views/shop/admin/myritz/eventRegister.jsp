 ...
 */
 $(document).ready(function() {
	 ...
	 $('.submitImg').on('click', function() {
		...
		if ($('[name=schMyritzNm]').val() == ''){
			alert("판매사를 선택 해주세요");
			$('[name=schMyritzNm]').focus();
			return false;
		}
		...
		var myritzCdValue = $('#schMyritzCd').val();
		if('' != myritzCdValue && myritzCdValue != null && myritzCdValue != undefined){
			$('#myritzcode').val(myritzCdValue);
		}
		
	});
	 
	//전체 적용 시 판매사 clear
	$("input[name='myritzRadio']").change(function(){
		if ($(this).val() === 'a') {
			$('#schMyritzCd').val('');
			$('#schMyritzNm').val('');
		}
...
================================================================================ --%>
		...
		<form method="post" action="myritzEventIndb" enctype="multipart/form-data">
			...
			<input type="hidden" name="myritzcode" id="myritzcode" value="${eventVO.eventObj.myritzcode}" />
			<table class=tb>	
				<col class=cellC><col class=cellL>
				<tr>
					<td>판매사</td>
					<td>
						<input type="text" name="schMyritzNm" id="schMyritzNm" value="${eventVO.eventObj.myritzname}" class="line" style="height:22px" readonly="readonly" />
						<input type="button" value="판매사 조회" onclick="popupLayer('${ctx}/shop/admin/common/myritzSearchPopup', 600, 500);" />
						<input type="hidden" name="schMyritzCd" id="schMyritzCd" value="${eventVO.eventObj.myritzcode}" />
					</td>
				</tr>
				<tr>
					<td>이벤트제목</td>
			...
			<div class=button>
				...
				<a href="${ctx}/shop/admin/myritz/myritzEventList"><img src="/resources/shop/admin/img/btn_cancel.gif"></a>
			</div>
