 ...
 */
 $(document).ready(function() {
	 ...
	 $('.submitImg').on('click', function() {
		var sellerValue = $('input:radio[name="sellerRadio"]:checked').val();
		if (sellerValue == 's' && $('[name=schSellerNm]').val() == ''){
			alert("판매사를 선택 해주세요");
...
================================================================================ --%>
		...
		<form method="post" action="indb" enctype="multipart/form-data">
			<input type=hidden name=mode value="${eventVO.mode }">
			<c:if test="${'modify' == eventVO.mode}">
			...
			<table class=tb>	
				...
				<tr>
					<td>판매사</td>
					<td>
						<input type=radio name="sellerRadio" id="sellerRadio1" value="a" <c:if test="${eventVO.eventObj.sellercode == null || '' == eventVO.eventObj.sellercode}">checked</c:if>>전체
						<br/>
						<input type=radio name="sellerRadio" id="sellerRadio2" value="s" <c:if test="${eventVO.eventObj.sellercode != null && '' != eventVO.eventObj.sellercode}">checked</c:if>>판매사 선택
			...
			<div class="button">
				<input type=image src="/resources/shop/admin/img/btn_${'modify' eq eventVO.mode ? 'modify' : 'register'}.gif" class="submitImg" />
				<a href="${ctx}/shop/admin/event/list"><img src="/resources/shop/admin/img/btn_cancel.gif"></a>
			</div>
		</form>
<%-- ================================================================================
* 업무 HTML CONTENT 종료
