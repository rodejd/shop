...
function checkform(form) {
	...
	if (form.priodtype.value == "0") {
		...
		$("#sdate").val($("#sdt").val().substring(0, 4) + "-" + $("#sdt").val().substring(4, 6) + "-" + $("#sdt").val().substring(6, 8) + " " + $("#shour").val() + ":" + $("#stime").val() + ":00");
		$("#edate").val($("#edt").val().substring(0, 4) + "-" + $("#edt").val().substring(4, 6) + "-" + $("#edt").val().substring(6, 8) + " " + $("#ehour").val() + ":" + $("#etime").val() + ":59");
	} else {
		if (form.priod.value == "") {
			alert("적용기간을 입력하세요.");
			form.priod.focus();
...
$(function() {
	...
	if ($('input:radio[name=coupontype]:checked').val() == 1) {
		chk_coupontype();
		openLayer("divdcnt");
	}else if ($('input:radio[name=coupontype]:checked').val() == 3) {
		chk_coupontype();
		openLayer("divdcode");
	}
});
...
<form id="couponForm" name="couponForm" method="post" action="indb.coupon" enctype="multipart/form-data" onsubmit="return checkform(this);">
	...
	<input type="hidden" id="sdate" name="sdate" value="${couponInfo.sdate}">
	<input type="hidden" id="edate" name="edate" value="${couponInfo.edate}">
	...
	<table class=tb>
		...
		<tr>
			<td>쿠폰설명</td>
			<td><input type=text name='summa' size=40 maxlength=70 value="${couponInfo.summa}" required class=line> <font class=extext> ex) 특가이벤트! 여름상품 10% 할인쿠폰</font></td>
		</tr>
		<tr>
			<td>쿠폰이미지</td>
			<td>
		...
		<tr>
			<td>쿠폰발급방식</td>
			<td>
				<div><input type=radio name=coupontype value='0' class=null <c:if test="${'0' == couponInfo.coupontype}">checked</c:if> onclick="chk_coupontype(); closeLayer('divdcnt'); closeLayer('divdcode');"> 운영자발급 <font class=extext>(쿠폰등록 후 쿠폰리스트에서 운영자가 특정회원에게 발급합니다)</font></div>
				<%-- <div><input type=radio name=coupontype value='1' class=null <c:if test="${'1' == couponInfo.coupontype}">checked</c:if> onclick="chk_coupontype(); openLayer('divdcnt'); closeLayer('divdcode');"> 회원직접다운로드 <font class=extext>(상품상세정보에서 회원이 직접 쿠폰을 다운로드받습니다)</font></div>
				...
				--%>
				<div><input type=radio name=coupontype value='2' class=null <c:if test="${'2' == couponInfo.coupontype}">checked</c:if> onclick="chk_coupontype(); closeLayer('divdcnt'); closeLayer('divdcode');"> 회원가입자동발급 <font class=extext>(회원가입시 자동발급됩니다)</font></div>
				<div><input type=radio name=coupontype value='3' class=null <c:if test="${'3' == couponInfo.coupontype}">checked</c:if> onclick="chk_coupontype(); closeLayer('divdcnt'); openLayer('divdcode');"> TEXT 입력 다운로드형 <font class=extext>(쿠폰코드 입력시 사용됩니다)</font></div> 
				<div><input type=radio name=coupontype value='4' class=null <c:if test="${'4' == couponInfo.coupontype}">checked</c:if> onclick="chk_coupontype(); closeLayer('divdcnt'); openLayer('divdcode');"> 즉시 적용쿠폰 <font class=extext>(다운로드 없이 즉시 쿠폰 적용됩니다)</font></div> 
				<div id='divdcode' style="padding:3 0 10;display:none;">
			    	<table border=1 bordercolor=#cccccc style="border-collapse:collapse" width=635>
						<tr>
							<td bgcolor=white style="padding:5 0 7 2">
								<div style='padding-left:10'>쿠폰 코드<input type='text' style='width: 100px; margin-left: 20px; text-align:right' name='dncode' size=3 value="${couponInfo.dncode}" maxlength='30'></div>
							</td>
						</tr> 
					</table>
				</div>
		...
		<tr>
			...
			<td>
				<table width=100% cellpadding=0 cellspacing=0>
					<tr>
						<td><input type=radio name=goodstype value='0' class=null <c:if test="${'0' == couponInfo.goodstype}">checked</c:if>> 전체상품에 발급합니다</td>
					</tr>
					<tr>
						<td height=10></td>
					</tr>
					...
					<tr>
						<td><input type=radio name=goodstype value='1' class=null <c:if test="${'1' == couponInfo.goodstype}">checked</c:if> > 특정 상품 및 특정 카테고리에 발급합니다 <font class=extext>(아래에서 검색후 선정)</font></td>
					</tr>
					<tr>
						<td height=5></td>
					</tr>
					<tr>
						<td>
							...
							<div style=padding-left:8><font class=small1 color=FF0066><img src="/resources/shop/admin/img/icon_list.gif" align="absmiddle">카테고리 선정 (카테고리선택 후 오른쪽 선정버튼클릭 , 쿠폰발급은 2차 분류만 적용가능)</font></div>
							<div style=padding-left:8><script>new categoryBox('cate',4,'','couponForm');</script>
							<a href="javascript:exec_add()"><img src="/resources/shop/admin/img/btn_coupon_cate.gif"></a></div>
							<div class="box" style="padding:10 0 0 10">
								...
								<c:if test="${couponCategoryArr != null &&  fn:length(couponCategoryArr) >0 }">	
									<c:forEach items="${couponCategoryArr}" var="cate" varStatus="i">
									<tr>
										<td id=currPosition>
											${shopLibFunction:getCurrPosition(cate, '0', 'kr') }<%--=getCurrPosition(dbHandler , rt_cat.get(i,"category"))--%>
											<input type="text" name="categoryArr" value="${cate}" style="display:none"></td>
										<td>
											<input type=hidden name=sortArr value="${i.index - 1}" class="sortBox right" maxlength=10 >
										</td>
										<td>
											<a href="javascript:void(0)" onClick="cate_del(this.parentNode.parentNode)">
											<img src="/resources/shop/admin/img/i_del.gif" border=0 align=absmiddle></a>
										</td>
									</tr>
									</c:forEach>
								</c:if>
							...
							<div id=divRefer style="position:relative;padding-left:8">
		...
		<tr>
			...
			<td>
				...
				<div id=priodid0 style="display:block;height:50;padding-top:10">
					<input type="text" id="sdt" name="sdt" value="${couponInfo.sdt}" onclick="calendar(event)" onkeydown="onlynumber(event);" onkeyup="removeHangul(event);" size="10"/>
					<c:set var="stm" value="${fn:split(couponInfo.stm,':')}" />
					...
					~
					<input type="text" id="edt" name="edt" value="${couponInfo.edt}" onclick="calendar(event)" onkeydown="onlynumber(event);" onkeyup="removeHangul(event);" size="10"/>
					<c:set var="etm" value="${fn:split(couponInfo.etm,':')}" />
		...
		<tr <c:if test="${couponVO.mode eq 'register'}">style="display:none"</c:if>>
			<td>승인여부</td>
			<td>
				상태 : ${couponInfo.approvalstatusnm}
				&nbsp;&nbsp;&nbsp;
				<input type="radio" name="statusRadio" id="approvalstatus_2" value="2" <c:if test="${'2' == couponInfo.approvalstatus or couponVO.mode eq 'register'}">checked</c:if>> 승인 
	...
	<div class=button>
		...
		<a href="${ctx}/shop/admin/event/coupon"><img src="/resources/shop/admin/img/btn_cancel.gif"></a>
	</div>
</form>
