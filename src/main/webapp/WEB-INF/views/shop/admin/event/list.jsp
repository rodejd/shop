...
================================================================================ --%>
		...
		<table width=100% cellpadding=0 cellspacing=0 border=0>
			<tr>
				<td>
					<form name="eventSchForm" id="eventSchForm" action="${ctx}/shop/admin/event/list" method="get">
...
<c:if test="${!empty eventVO.eventList}">			
	<c:forEach items="${eventVO.eventList}" var="eventList" varStatus="st">		
			...
			<tr height=25>
				...
				<td align=center style="padding-bottom:9"><font class=ver81>${eventList.sellername}</font></td>
				<td align=center style="padding-bottom:9"><font class=ver81><img src="/resources/shop/admin/img/icn_${eventList.open}.gif"></font></td>
				<td align=center style="padding-bottom:9">
					<font class=ver81>
				...
				<td align=center style="padding-bottom:4">
					<a href="register?mode=modify&sno=${eventList.sno}"><img src="/resources/shop/admin/img/i_edit.gif" border=0></a>
					<a href="indb?mode=delete&sno=${eventList.sno}&old_attach=${eventList.attach}" onclick="return confirm('정말로 삭제하시겠습니까?\n\n이벤트내용에 쓰인 이미지는 다른 곳에서도 사용하고 있을 수 있으므로 자동 삭제되지 않습니다.')"><img src="/resources/shop/admin/img/i_del.gif"></a>
					<%-- \n\'디자인관리 > webFTP이미지관리 > data > editor\'에서 이미지체크 후 삭제관리하세요.' 삭제문구 뒤에 인용한 문구 현재 디자인관리가 없기 때문에 따로 뺴서 주석처리 --%>
				</td>
			</tr>
	</c:forEach>	
</c:if>	
<c:if test="${empty eventVO.eventList}">
		...
		<div id=MSG01>
			...
			<tr><td><img src="/resources/shop/admin/img/icon_list.gif" align=absmiddle>이벤트제목을 클릭하면 이벤트 내용을 수정 할 수 있습니다.</td></tr>
			<tr><td><img src="/resources/shop/admin/img/icon_list.gif" align=absmiddle>미리보기를 클릭하면 새창과 함께 이벤트페이지로 이동 합니다.</td></tr>
			</table>
		</div>
