...
function delCoupon(couponcd){
	...
	if(confirm('쿠폰을 삭제하시면 발급정보들도 같이 사라집니다.\n삭제 후 원래 쿠폰정보는 복구하실 수 없습니다.\n정말 삭제하시겠습니까?')){
		...
		ajaxCallJsonPost("/shop/admin/event/indb.coupon", "couponSchForm", function(data){
			if (data.RESULT) {
				alert('삭제하였습니다.');
				window.location.href = ctx+'/shop/admin/event/coupon';
			} else {
...
function chk_status_modify(statVal, mode){
	...
	if(confirm(("open_modify" == mode) ? "게시/게시취소 하시겠습니까?" : "승인/반려 하시겠습니까?")){
		...
		$("#couponSchForm").attr("action", "indb.coupon2");
...
function goPage(page){
	$("#couponSchForm").attr("action", ctx+"/shop/admin/event/coupon");
...
<table width=100% cellpadding=0 cellspacing=0 border=0>
	<tr>
		<td>
			<form name="couponSchForm" id="couponSchForm" action="${ctx}/shop/admin/event/coupon" method="get">
				...
				<table class=tb>
					...
					<tr>
						...
						<td colspan=3>
							<input type="text" name="schSellerNm" id="schSellerNm" value="${couponVO.schSellerNm}" class="line" style="height:22px" />
							<input type="button" value="판매사 조회" onclick="popupLayer('${ctx}/shop/admin/common/sellerSearchPopup', 600, 500);" />
							<input type="hidden" name="schSellerCd" id="schSellerCd" value="${couponVO.schSellerCd}" />
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td colspan=3>
							<font class=small1 color=555555>
								...
								<input class=null type=checkbox name='coupontype' value='4' ${stringUtil:checkedArry(couponVO.coupontype, '4')} >즉시 적용쿠폰
							</font>
						</td>
					</tr>
					<tr>
...
<c:if test="${!empty couponVO.couponList}">	
	...
	<tr height=35>
		...
		<td align=center><font class=small1 color=555555>${list.couponcd}</font></td>
		...
		<td align=center>
			<font class=small1 color=0074BA><b>
				<c:choose>
					...
					<c:when test="${'4' == list.coupontype}">즉시 적용쿠폰</c:when>
		...
		<td align=left style="padding-left:5">
			<font class=small1 color=555555>
				<a href="${ctx}/shop/admin/event/coupon_apply?couponcd=${list.couponcd }">
				<c:choose>
		...
		<td align=center style="font-family: verdana;font-size:7pt;letter-spacing:-1">
			<font class=ver71 color=555555>${list.sellerNameList}</font>
		</td>
		<td align=center style="font-family: verdana;font-size:7pt;letter-spacing:-1">
		...
		<td align=center>
			<font class=small1 color=555555>
				<a href="${ctx}/shop/admin/event/coupon_register?couponcd=${list.couponcd}&mode=modify"><img src="/resources/shop/admin/img/i_edit.gif"></a> 
...
<c:if test="${empty couponVO.couponList}">
	<tr><td align="center" colspan="15" style="padding-bottom:9;padding-top:9"><font class="ver81">검색 결과가 없습니다.</font></td></tr>
</c:if>
