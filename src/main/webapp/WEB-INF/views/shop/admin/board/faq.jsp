...
<div class="title title_top">FAQ관리<span>고객들이 자주하는 질문을 미리 예상해서 작성합니다</span> <!-- <a href="javascript:popup('http://guide.godo.co.kr/season2/board/view.jsp?id=board&no=8',870,800)"><img src="../img/btn_q.gif" border=0 align=absmiddle hspace=2></a> --></div>
<table class=tb>
...
<tr>
	...
	<td>
	<select name="skey">
	<option value="all"  ${stringUtil:selected(faqVO.skey,"all") }> 통합검색 </option>
	<option value="question"  ${stringUtil:selected(faqVO.skey, "question")}> 질문 ( 단문 ) </option>
	<option value="descant"  ${stringUtil:selected(faqVO.skey, "descant")}> 질문 ( 장문 ) </option>
	<option value="answer"  ${stringUtil:selected(faqVO.skey, "answer")} > 답변 </option>
	</select> <input type="text" NAME="sword" value="${faqVO.sword }" class=line>
	</td>
	<td>분류선택</td>
	<td>
	<select name="sitemcd">
	<option value="all" ${stringUtil:selected(faqVO.sitemcd, "all")}> - 전체 - </option>
	
	${webUtil:makeSelectCodeItem((codeUtil:codeitem('faq')), faqVO.sitemcd) }	
	</select>
	</td>
</tr>
<tr>
	<td>등록일</td>
	<td>
	<input type=text name=sregdt value="${faqVO.sregdt_0 }" onclick="calendar(event)" class=line onkeydown="onlynumber()"> -
	<input type=text name=sregdt value="${faqVO.sregdt_1 }" onclick="calendar(event)" class=line onkeydown="onlynumber()">
	...
	<td>
		<select name="schSkin">
			...
			${webUtil:makeSelectCodeItem((codeUtil:codeitem('skin')), faqVO.schSkin) }
		</select>
	</td>
</tr>
</table>
...
<tr>
	...
	<c:set var="pages" value="${(faqVO.rowCount*10) / (faqVO.pageSize*10)} " />
	<c:set var="pageCnt" value="${pages+(1-(pages%1))%1}" />
	<fmt:parseNumber var="var3" value="${pageCnt}" integerOnly="true" />
	총 <b>${faqVO.totalCount}</b>개, 검색 <b>${faqVO.rowCount}</b>개, <b>${faqVO.pageNo}</b>of <b>${var3}</b> Pages
	</td> 
	<td align=right>
	<select name="searchsort" onchange="this.form.submit();">
	<option value="regdt desc" ${stringUtil:selected(faqVO.searchsort, "regdt desc")}>- 등록일 정렬↑</option>
	<option value="regdt asc" ${stringUtil:selected(faqVO.searchsort, "regdt asc")} >- 등록일 정렬↓</option>
	<option value="sort desc" ${stringUtil:selected(faqVO.searchsort, "sort desc")} >- 출력순서 정렬↑</option>
	<option value="sort asc"  ${stringUtil:selected(faqVO.searchsort, "sort asc")}>- 출력순서 정렬↓</option>
    <optgroup label="------------"></optgroup>
	<option value="question desc" ${stringUtil:selected(faqVO.searchsort, "question desc")} >- 질문( 단문 ) 정렬↑</option>
	<option value="question asc" ${stringUtil:selected(faqVO.searchsort, "question asc")} >- 질문( 단문 ) 정렬↓</option>
	</select>&nbsp;

	<select name=pageSize onchange="this.form.submit()">
		<option value="10" ${stringUtil:selected(faqVO.pageSize, "10")} >10개 출력
		<option value="20" ${stringUtil:selected(faqVO.pageSize, "20")} >20개 출력
		<option value="40" ${stringUtil:selected(faqVO.pageSize, "40")} >40개 출력
		<option value="60" ${stringUtil:selected(faqVO.pageSize, "60")} >60개 출력
		<option value="100" ${stringUtil:selected(faqVO.pageSize, "100")} >100개 출력
		</select>
	</td>
</tr>
</table>
...
<form method="post" action="" name="fmList" id="fmList">
...
<tr><td class=rnd colspan=10></td></tr>
	...
	<c:choose>
		<c:when test="${faqVO.faqList != null &&  fn:length(faqVO.faqList) >0 }">
			<c:forEach items="${faqVO.faqList }" var="list" varStatus="vnum">
				<tr><td height=4 colspan=10>
				<input type="hidden" name="sno" value="${list.sno }" />
				</td></tr>
				<tr height=25 align="center">
				
					<td>${(faqVO.rowCount - vnum.index) - ( faqVO.rowStart ) }</td>
					<td>
					${codeUtil:getCodeName("faq", list.itemcd)}
<%-- 					<select name="itemcd" style="width:100;">
					<option value=""> - 분류 - </option>
					${webUtil:makeSelectCodeItem((codeUtil:codeitem('faq')), list.itemcd) }
					</select> --%>
					</td>
					<td align=left><a href="register?mode=modify&sno=${list.sno}">${list.question}</a></td>
					<%-- <td><font class=ver8 color=444444>${list.regdt}</td> --%>
					...
					<%-- <input type="text" size="25" name="sort" value="${list.sort}" style="width:30;text-align:center" readonly> --%></td>
					<td STYLE="PADDING-TOP:3PX;"><a href="register?mode=modify&sno=${list.sno }"><img src="/resources/shop/admin/img/i_edit.gif"></a></td>
		...
		<c:otherwise>
			...
			<tr height=25><td align=center colspan=8> NO_LIST_DATA </td></tr>
...
<div align=center class=pageNavi>
	<b><tags:paginator currentPageNo="${faqVO.pageNo}" rowCount="${faqVO.rowCount}" pageSize="${faqVO.pageSize}"  pageGroupSize="${faqVO.pageGroupSize}" /></b>
</div>

<div style="float:left;"><input type="image" src="/resources/shop/admin/img/btn_allselect_s.gif" 	alt="전체선택" border="0" align='absmiddle'  
		<c:if test="${fn:length(faqVO.faqList)!=0}">
