...
$(function(){
	...
	$('#couponSubmitBtn').on('click', function(){
		if (checkform1($('#couponAddForm')[0])) {
			...
			ajaxCallJsonPost("/shop/admin/seller/sellerIndbCoupon", "couponAddForm", function(data){
				if (data.RESULT) {
					alert('등록하였습니다.');
	...
	$('#modifyApplyBtn').on('click', function(){
		...
		ajaxCallJson("/shop/admin/seller/sellerIndbCoupon", 
				{mode:"modifyApply",
				snoGroup:snoGroup,
...
function delApply(sno){
	...
	f.sno.value = sno;
	ajaxCallJsonPost("/shop/admin/seller/sellerIndbCoupon", "hiddenform", function(data){
		if (data.RESULT) {
			alert('삭제하였습니다.');
			location.reload(true);
		} else {
...
<table class="tb" border="1" bordercolor="#e6e6e6" cellpadding="5" style="width: 100%; border-collapse: collapse;">
	...
	<tr height=25>
		...
		<td class=cellL align=center>
			<a href="${ctx}/shop/admin/seller/sellerCouponRegister?couponcd=${couponInfo.couponcd}&mode=modify" target='_blank'><font color=0074BA>${couponInfo.coupon}</font></a>
		</td>
		<td align=center>
			<c:choose>
	...
	<c:choose>
		<c:when test="${couponVO.couponApplyGrpList != null &&  fn:length(couponVO.couponApplyGrpList) >0 }">	
			...
			<tr heigth="25">
				...
				<a href="javascript:popupLayer('${ctx}/shop/admin/seller/sellerPopupApply?sno=${grplist.sno }', 500, 500)" >
				<c:choose>
			...
			<td valign="top">
				<table class="tb" border="1" bordercolor="#e6e6e6" cellpadding="5" style="width: 100%; border-collapse: collapse;">
					...
					<tr>
						...
						<td valign="top">
							<div style="padding-top:4"><a href="javascript:popup('sellerPopupMember',800,600);"><img src="/resources/shop/admin/img/arrow_blue.gif" align=absmiddle><font color=0074BA><b>[회원검색하기]</b></font></a></div>
							<div class="box">
