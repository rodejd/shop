...
function delCoupon(couponcd){
	...
	if(confirm('쿠폰을 삭제하시면 발급정보들도 같이 사라집니다.\n삭제 후 원래 쿠폰정보는 복구하실 수 없습니다.\n정말 삭제하시겠습니까?')){
		...
		ajaxCallJsonPost("/shop/admin/seller/sellerIndbCoupon", "couponSchForm", function(data){
			if (data.RESULT) {
				alert('삭제하였습니다.');
				window.location.href = ctx+'/shop/admin/seller/sellerCouponList';
			} else {
...
function chk_status_modify(statVal, mode){
	...
	if(confirm(("open_modify" == mode) ? "게시/게시취소 하시겠습니까?" : "승인/반려 하시겠습니까?")){
		...
		$("#couponSchForm").attr("action", "sellerIndbCoupon2");
...
$(function(){
	...
	$('#searchBtn').on('click', function(){
		$("#couponSchForm").attr("action", ctx+"/shop/admin/seller/sellerCouponList");
...
<table width=100% cellpadding=0 cellspacing=0 border=0>
	<tr>
		<td>
			<form name="couponSchForm" id="couponSchForm" action="${ctx}/shop/admin/seller/sellerCouponList" method="get">
...
<c:if test="${!empty couponVO.couponList}">	
	...
	<tr height=35>
		...
		<td align=left style="padding-left:5">
			<font class=small1 color=555555>
				<a href="${ctx}/shop/admin/seller/sellerCouponApply?couponcd=${list.couponcd }">
				<c:choose>
		...
		<td align=center>
			<font class=small1 color=555555>
				<a href="${ctx}/shop/admin/seller/sellerCouponRegister?couponcd=${list.couponcd}&mode=modify"><img src="/resources/shop/admin/img/i_edit.gif"></a> 
