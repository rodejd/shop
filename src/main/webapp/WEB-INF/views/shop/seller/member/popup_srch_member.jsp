...
function goPage(page){
	window.location.href="popup_srch_member?schType=${sellerSmsFM.schType}&schWord=${sellerSmsFM.schWord}&pageNo="+page;
}
...
<tr>
	<td>
	<select name="schType">
	<option value="name" ${sellerSmsFM.schType == 'name' ? "selected" : "" }>이름
	<option value="m_id" ${sellerSmsFM.schType == 'm_id' ? "selected" : "" }>아이디
	</select>
	</td>
	<td>
	<input type=text name=schWord value="${sellerSmsFM.schWord}" class="line">
	</td>
...
<c:choose>
<c:when test="${sellerSmsFM.gdMemberList != null && fn:length(sellerSmsFM.gdMemberList) >0 }">	
<c:forEach items="${sellerSmsFM.gdMemberList}" var="gdMemberList" varStatus="st" >
<tr height=24 align="center">
	<td><font class=ver8 color=262626>${(sellerSmsFM.rowCount - st.index) - ( (sellerSmsFM.pageNo -1) * 10 ) }</td>
