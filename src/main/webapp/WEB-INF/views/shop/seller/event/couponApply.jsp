...
$(function(){
	...
	$('#couponSubmitBtn').on('click', function(){
		if (checkform1($('#couponAddForm')[0])) {
			...
			ajaxCallJsonPost("/shop/seller/event/selIndbCoupon", "couponAddForm", function(data){
				if (data.RESULT) {
					alert('등록하였습니다.');
	...
	$('#modifyApplyBtn').on('click', function(){
		...
		ajaxCallJson("/shop/seller/event/selIndbCoupon", 
				{mode:"modifyApply",
				snoGroup:snoGroup,
...
function delApply(sno){
	...
	f.couponcd.value = '${couponInfo.couponVO.couponcd}';
	f.sno.value = sno;
	ajaxCallJsonPost("/shop/seller/event/selIndbCoupon", "hiddenform", function(data){
		if (data.RESULT) {
			alert('삭제하였습니다.');
			location.reload(true);
		} else {
...
<form name="pageform" id="pageform" method="post">
	<input type="hidden" name="couponcd" value="${couponInfo.couponVO.couponcd}">
...
<table class="tb" border="1" bordercolor="#e6e6e6" cellpadding="5" style="width: 100%; border-collapse: collapse;">
	...
	<tr height=25>
		<td align=center><font class=ver71 color=444444>${couponInfo.couponVO.couponcd}</font></td>
		<td class=cellL align=center>
			<a href="${ctx }/shop/seller/event/couponRegister?schCouponcd=${couponInfo.couponVO.couponcd}&mode=modify" target='_blank'><font color=0074BA>${couponInfo.couponVO.coupon}</font></a>
		</td>
		<td align=center>
			<c:choose>
				<c:when test="${'0' == couponInfo.couponVO.coupontype}">운영자발급</c:when>
				<c:when test="${'1' == couponInfo.couponVO.coupontype}">회원직접다운로드</c:when>
				<c:when test="${'2' == couponInfo.couponVO.coupontype}">회원가입자동발급</c:when>
				<c:when test="${'3' == couponInfo.couponVO.coupontype}">구매후 자동발급</c:when>
				<c:otherwise>&nbsp;</c:otherwise>
			</c:choose>
		</td>
		<td align=center><font class=ver71 color=444444>${couponInfo.couponVO.regdt}</font></td>
		<td align=center><font class=ver71 color=444444>${couponInfo.couponVO.price}<c:if test="${!fn:endsWith(couponInfo.couponVO.price, '%')}">원</c:if></font></td>
		<td align=center>
			<font class=ver71 color=444444>
				<c:choose>
					<c:when test="${'1' == couponInfo.couponVO.priodtype}">발급 후 ${couponInfo.couponVO.sdate } 일</c:when>
					<c:otherwise>${couponInfo.couponVO.sdate }<br/>~${couponInfo.couponVO.edate }</c:otherwise>
				</c:choose>
			</font>
		</td>
		<td align=center>
			<font class=small1 color=444444>
				<c:choose>
					<c:when test="${'0' == couponInfo.couponVO.ability}">할인</c:when>
					<c:when test="${'1' == couponInfo.couponVO.ability}">적립</c:when>
					<c:otherwise>&nbsp;</c:otherwise>
				</c:choose>
			</font>
		</td>
...
<table class=tb border="1" bordercolor="#e6e6e6" cellpadding="5" style="width: 100%; border-collapse: collapse;">
	<tr class=cellC align=center height=25>
		...
		<c:if test="${'1' == couponInfo.couponVO.coupontype}">
		...
		<th align=center><!-- 삭제 -->쿠폰사용여부</th>
	</tr>
	
	<c:choose>
		<c:when test="${couponInfo.couponApplyGrpList != null &&  fn:length(couponInfo.couponApplyGrpList) >0 }">	
			<c:forEach items="${couponInfo.couponApplyGrpList}" var="grplist" varStatus="i">
			<tr heigth="25">
				...
				<c:choose>
					...
					<c:when test="${'1' == grplist.membertype}"><a href="javascript:popup('${ctx }/shop/seller/event/selPopupApply?popView=Y&sno=${grplist.sno}', 500, 300);" >${grplist.grpnm }</a></c:when>
					<c:when test="${'2' == grplist.membertype}"><a href="javascript:popup('${ctx }/shop/seller/event/selPopupApply?popView=Y&sno=${grplist.sno}', 500, 300);" >개별회원</a></c:when>
					<c:otherwise>&nbsp;</c:otherwise>
				</c:choose>
				</td>
				<td align=center>
		...
		<c:otherwise>
			<tr height=25>
			   <td align=center colspan=<c:choose><c:when test="${'1' == couponInfo.couponVO.coupontype}">7</c:when><c:otherwise>6</c:otherwise></c:choose>><font class=small1 color=6d6d6d>현재 이 쿠폰을 발급받은 회원이 없습니다.</font></td>
			</tr>
		</c:otherwise>
	</c:choose>
</table>
<c:if test="${'Y' == couponInfo.popView}">
<div style="text-align:center; padding-top:30px;">
	<input type="button" class="btn btn-primary" value="닫기" onclick="self.close();" />
</div>
</c:if>
<%-- 페이징 시작 --%>
<tags:paginator currentPageNo="${couponInfo.pageNo}" rowCount="${couponInfo.rowCount}" pageSize="${couponInfo.pageSize}"  pageGroupSize="${couponInfo.pageGroupSize}" />
<%-- 페이징 끝 --%>

<p>
<c:if test="${'0' == couponInfo.couponVO.coupontype && 'Y' != couponInfo.popView}">
			...
			<td valign="top">
				<table class="tb" border="1" bordercolor="#e6e6e6" cellpadding="5" style="width: 100%; border-collapse: collapse;">
					...
					<tr>
						...
						<td>전체회원(현재 총 <font color="EA0095"><b>${couponInfo.couponGrpTotal}명</b></font>)에게 쿠폰을 발급합니다.</td>
					</tr>
					<tr>
						...
						<td>
							<select name="membergrpsno" >
								...
								<c:choose>
								<c:when test="${couponInfo.couponGrpList != null &&  fn:length(couponInfo.couponGrpList) >0 }">	
									<c:forEach items="${couponInfo.couponGrpList}" var="list" varStatus="i">
					...
					<tr>
						...
						<td valign="top">
							<div style="padding-top:4"><a href="javascript:popup('selPopupMember',800,600);"><img src="/resources/shop/admin/img/arrow_blue.gif" align=absmiddle><font color=0074BA><b>[회원검색하기]</b></font></a></div>
							<div class="box">
...
<div id=MSG01>
	<table cellpadding=2 cellspacing=0 border=0 class=small_ex>
		<!-- <tr><td><img src="/resources/shop/admin/img/icon_list.gif" align=absmiddle>삭제버튼을 클릭하면 해당 회원에게 발급된쿠폰이 취소됩니다.</td></tr> -->
		<tr><td><img src="/resources/shop/admin/img/icon_list.gif" align=absmiddle>'쿠폰사용완료'란 해당 회원이 이미 쿠폰을 사용하여 완료됨을 의미합니다.</td></tr>
	</table>
</div>
