...
* 업무 HTML CONTENT 시작
================================================================================ --%>
		<form name=list action="orderRepay">
		...
		<form method="post" action="repay/indb" id="form01" name="form01">
			...
			<table width="100%" cellpadding="2" cellspacing="0">
				...
				<c:choose>
					<c:when test="${not empty repayVO.repayList }">
						<c:forEach items="${repayVO.repayList}" var="repayList" varStatus="status">
							...
							<tr>
								...
								<td>
									<a href="javascript:popup('${ctx }/shop/seller/order/popupOrder?ordno=${repayList.oCordno}',800,600)"><font class="ver71" color="0074BA"><b>${repayList.oCordno}</b></font></a>
								</td>
								<td>
							...
							<tr>
								<td colspan="10" style="padding:5px 10px" align="left">
									<table width="100%" border="1" bordercolor="#dedede" style="border-collapse:collapse">
										...
										<col align="center" span="10">
										<tr>
											<td><font class="ver7" color="444444"><fmt:formatNumber value="${repayList.sSumGoodsPrice}" pattern="#,###.##"/>원</font></td>
											<td><font class="ver7" color="444444"><fmt:formatNumber value="${repayList.sDelivery}" pattern="#,###.##"/>원</font></td>
											<td><font class="ver7" color="444444"><fmt:formatNumber value="${repayList.sAddDelivery}" pattern="#,###.##"/>원</font></td>
											<td><font class="ver7" color="444444"><fmt:formatNumber value="${repayList.sSumMemberDC}" pattern="#,###.##"/>원</font></td>
											<td><font class="ver7" color="444444"><fmt:formatNumber value="${repayList.sSumCoupon}" pattern="#,###.##"/>원</font></td>
											<td><font class="ver7" color="444444"><fmt:formatNumber value="${repayList.sSumSettlePrice}" pattern="#,###.##"/>원</font></td>
										</tr>
									</table>
								</td>
							</tr>
								...
								<td colspan="10" style="padding:5px 10px" align="left">
									<table width="100%" border="1" bordercolor="#dedede" style="border-collapse:collapse">
										<tr bgcolor="#f7f7f7" height="20">
											...
											<th width="160" nowrap><font class="small1" color="444444"><b>옵션</b></font></th>
													...
													<tr>
														...
														<c:choose>
															<c:when test="${subCatetory eq '001' }">
																...
																<font class="small" color="0074BA">${repayList.oIgoodsnm} [판매사 : ${repayList.sellerNm }]</font>
									...
									<table width="100%" border="1" bordercolor="#dedede" style="border-collapse:collapse">
										<tr bgcolor="#f7f7f7" height="20">
											...
											<th width="33%" nowrap><font class="small1" color="444444"><b>환불수수료</b></font> <a href="javascript:popupLayer('${ctx }/shop/seller/basic/popup_refundFee',600,300)"><img src="/resources/shop/admin/img/btn_repay_price.gif" border="0"></a></th>
										...
										<tr>
											<td align="center"><font class="ver7" color="0074BA"><b><fmt:formatNumber value="${repayList.refundPrice}" pattern="#,###.##"/>원</b></font></td>
											<c:set var="minrepayfee" value="${shopLibFunction:getProperty('minrepayfee')}"/>
											<td><font class="ver7" color="424242"><input type="text" name="repayfees" class="noline" value="${minrepayfee eq '' ? 0 : minrepayfee}" onchange="cal_repay(this.value,${repayList.refundPrice},${status.index})" onkeydown="onlynumber()" style="text-align=right;background:#E9FFB3">원</font></td>
										...
										<script>cal_repay(${minrepayfee eq '' ? 0 : minrepayfee},${repayList.refundPrice},${status.index});</script>
									</table>
					...
					<c:otherwise>
						<tr bgcolor="#FFFFFF" height="25">
							<td align="center" colspan="8"> 주문취소 내역이 존재하지 않습니다. </td>
						</tr>
					</c:otherwise>
				</c:choose>
			</table>
	...
	<tags:paginator currentPageNo="${repayVO.pageNo}" rowCount="${repayVO.rowCount}" pageSize="${repayVO.pageSize}"  pageGroupSize="${repayVO.pageGroupSize}" />
		...
		<div id="MSG01">
			<table cellpadding="1" cellspacing="0" border="0" class="small_ex">
				<tr><td>입금한 상태에서 주문을 취소하거나 이미 배송되어 반품시 발생하는 환불건에 대해 완료처리하는 영역입니다.</td></tr>
				<tr><td>주문취소와 반품완료처리를 통해 환불접수된 주문건이 환불접수리스트에 보입니다.</td></tr>
				<tr><td>환불접수건을 확인하고 고객의 통장으로 환불금액을 입금합니다.</td></tr>
				<tr><td>환불입금이 완료된 해당 주문건을 선택한 후 환불완료처리합니다.</td></tr>
				<tr><td>최종 환불금액이란 실결제금액에서 환불수수료를 제한 금액을 말합니다.</td></tr>
				<tr><td>환불수수료란 반품으로 인해 발생된 반송비용 및 기타 수수료를 의미하며, 기본값을 설정할 수 있습니다.</td></tr>
				<tr><td>적립금으로 결제한 경우 환불적립금을 정산하여 재적립 해주어야 합니다.</td></tr>
			</table>
		</div>
