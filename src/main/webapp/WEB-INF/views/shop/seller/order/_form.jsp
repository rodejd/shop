...
<script>
	...
	{
		if($("#layer_cancel").css("display") == "none"){		
			var sno = new Array();
...
<form name=frmOrder action="${ctx }/shop/seller/order/updateOrder" method=post>
	...
	<input type=hidden name=userNm value="${popupVO.rtData.mId}">
	<input type=hidden name=email value="${popupVO.rtData.email}">
...
<!-- 		<iframe id=ifrmCancel name=ifrmCancel style="width:100%;height:0;" frameborder=0></iframe> -->
				...
				<td>		
					<table class=tb cellpadding=4 cellspacing=0>
						...
						<c:forEach items="${popupVO.tmpRt9}" var="tmpRt9" varStatus="status">
							<input type="hidden" name="cancel" value="${tmpRt9.cancel}"/>
							<tr>
								<td  style="padding:2px 10px" rowspan=2 align=center><font class=ver7 color=444444>${status.count}</font></td>
								<td align=center><font class=ver8><fmt:formatNumber value="${tmpRt9.rfee}" pattern="#,###"/>원</font></td>
								<td align=center><font class=ver8 color=EA0095><b><fmt:formatNumber value="${tmpRt9.rprice}" pattern="#,###"/></b></font>원</td>
								<td align=center><font class=ver8><fmt:formatNumber value="${tmpRt9.remoney}" pattern="#,###"/></font>원</td>
	...
	<c:choose>
		<c:when test="${ 'view' == param.viewFlg }">
			<a href="${ctx }/shop/seller/order/list?mode=group&period=0&first=1"><img src="/resources/shop/admin/img/btn_list.gif"></a>
		</c:when>
		<c:otherwise>
