					...
					<tr bgcolor="${rt.bgColor}">
						<td width=35 nowrap class=noline>
							<input type=checkbox name=chk[] value="${rt.sno}" 
								<c:if test="${(rt.istep >= 41 and rt.istep <= 44) or (60 < rt.istep and rt.istep < 70) or 70 < rt.istep}">
									disabled
								</c:if>
							/>
						</td>
						...
						<td width=350 nowrap>
							<a href="javascript:popup('${ctx }/shop/seller/goods/register?popView=Y&goodsno=${rt.goodsno}',825,600)">
						...
						<%-- 소계 --%>
						<td width=55 nowrap>
							<fmt:formatNumber value="${(rt.price* rt.ea)+rt.addoptPrice}" pattern="#,###"/>
						</td>
						<%-- 판매사 결제가격 --%>
						<td width=55 nowrap><fmt:formatNumber value="${(popupVO.goodsPrice+popupVO.rtData.delivery+popupVO.rtData.addDelivery) - (popupVO.memberDc+popupVO.coupon)}" pattern="#,###"/></td>						
						<td nowrap>
							<c:choose>
								...
								<c:otherwise>
									<span class="_delivery-price _delivery-price${rt.goodsno}" data-goodsno="${rt.goodsno}">
