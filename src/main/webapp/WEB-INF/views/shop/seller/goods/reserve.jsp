		...
		<c:forEach items="${rtList}" var="list" varStatus="status">
			...
			<c:choose>
				<c:when test="${list.link eq '1'}">
					<td><a href="javascript:popup('${ctx }/shop/seller/goods/register?mode=modify&goodsno=${list.goodsno }&popView=Y',825,600);"><font class=small1 color=0074BA>${list.goodsnm !=null ? list.goodsnm : ''}</a></td>
				</c:when>
				<c:otherwise>
